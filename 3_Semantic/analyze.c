...
static void checkNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		case ReturnStmt:
		{
			...
			if(t->child[0] != NULL && currentScope->func->type != t->child[0]->type) InvalidReturnError(t->lineno);
		...
		case CallExpr:
		{
			...
			if(argNode == NULL && paramNode->type != Void) InvalidFunctionCallError(t->name, t->lineno);
